...
let nuevoObjeto;
let nuevObjetoJSON;
//captura el boton de busqueda
const buscadorboton = document.querySelector('#buscadorboton');
const searchbox = document.querySelector(".cajadebusqueda");
//Esta i es para El edentificador del boton 
let i=0;
localStorage.setItem("confi", 1)
...
const traerProductos = async()=>{
  const res = await fetch("../data.json");//IMPORTAR EL JSON
...
//------------      
   ...
   const capturarPalabras=()=>{
    const captura = document.querySelector('#buscadorinput').value;
    let palabra = captura.toLowerCase();
    let arbalap = palabra[0].toUpperCase();
    let restoDepalabra = palabra.slice(1);
    let palabraCompleta= arbalap+restoDepalabra;
    buscador(palabraCompleta);
   };


   const buscador = async(datos)=>{//busca el valor que va a recibir como parametro
    let paso= await fetch("../data.json");
    let data = await paso.json()
    let palabra = data.filter(element => element.objeto === datos)//obtengo el nombre
        let resultado = palabra;
        
         /******************************************* */
         

       resultado.forEach((objetos)=>{
//traemos el producto     
        const caja = document.createElement("div");//CREO LAS CAJITAS
        //AGREGO CLASES A LAS CAJA
        ...
        <p class="card-text">$${objetos.precio}</p>
        <p class="card-text">${objetos.cantidad}</p>
        <a class="btn btn-primary p d">comprar</a>
        </div>
        </div>
        <div> 
        `;
        ...
        d.addEventListener("click", ()=>{
          let padre = d.parentNode.children;
          punchs(padre);//Ejecuto la funcion punchs
    ...
    //FUNCION QUE ELIMINA EL PRODUCTO QUE YA ESTABA EN LA BUSQUEDA
    const buscadorinput = document.querySelector("#buscadorinput").addEventListener("click",()=>{
      let p= document.querySelector('.p');
      let falso = JSON.parse(localStorage.getItem("confi"));
      if(falso === 0){
        p.remove()
        localStorage.setItem("confi", 1)
   ...
   buscadorboton.addEventListener("click", ()=>{
    let verdad = JSON.parse(localStorage.getItem("confi"));
    if(verdad === 1){
      capturarPalabras();
      localStorage.setItem("confi", 0)
   }
